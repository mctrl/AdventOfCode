...
var test = `[1518 - 11 - 05 00: 45]falls asleep
[1518 - 11 - 03 00: 24]falls asleep
[1518 - 11 - 04 00: 02]Guard #99 begins shift
[1518 - 11 - 01 00: 05]falls asleep
[1518 - 11 - 01 00: 55]wakes up
...
listOfTimes = `[1518-06-12 23:57] Guard #2633 begins shift
...
[1518-06-15 00:55] wakes up`.split('\n').sort().map(value => {
    var date = (/\d{4}-\d{2}-\d{2}/).exec(value);
    var time = (/\d{2}:\d{2}/).exec(value);
    var id = (/#\d+/).exec(value) ;
    var action = (/[a-zA-Z]+/).exec(value);
    return {'date': date[0], 'time' : time[0] , 'action': action[0], 'id': id ? id[0] : null};
});
...
listOfTimes.forEach(record => {
    id = record.id || id; //if theres an ID keep it, otherwise update it
    if (record.action === "falls") {
        falls = parseInt(record['time'].split(':')[1]);
    } else if (record.action === "wakes") {
        awakes = parseInt(record['time'].split(':')[1]);
    }
    if (falls && awakes) {
        var newAsleepTime = awakes - falls;
        // console.log(id, ' ', awakes ,'-', falls,' ', newAsleepTime);
        guards[id] = guards[id] || { 'shiftPattern': new Array(60).fill(0)};
        guards[id].time = (guards[id].time || 0 )+newAsleepTime;
        
        for (let index = falls; index < awakes; index++) {
            // console.log('writing asleep time for guard ', id, ' from minute ', falls, ' to minute ', awakes, ' - ', index);
            guards[id]['shiftPattern'][index]++;
            // console.log(index,guards[id]['shiftPattern'])
        }

        if (guards[id].time > worstGuard.time) {

            worstGuard.id = parseFloat(id.split('#')[1]);
            worstGuard.time = guards[id].time;
            worstGuard.minute = guards[id].shiftPattern.indexOf(Math.max(...guards[id].shiftPattern)); // Math.min.apply(Math, guards[id].shiftPattern)
        }
        awakes = null;
        falls = null;
    }
